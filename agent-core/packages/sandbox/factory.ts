...
export async function connectSandbox(
  ...
  if (isNewApi) {
    ...
    if (config.state.type === "local") {
      return connectLocal(config.state, config.options);
    }
    return connectVercel(config.state as { type: "vercel" } & VercelState, config.options);
  }

  const state = configOrState as SandboxState;
  if (state.type === "local") {
    return connectLocal(state, legacyOptions);
  }
