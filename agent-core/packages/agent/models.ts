...
export function gateway(
  ...
  // Custom LLM provider from env vars
  if (process.env.LLM_API_KEY && process.env.LLM_API_URL) {
    const customOpenAI = createOpenAI({
      baseURL: process.env.LLM_API_URL,
      apiKey: process.env.LLM_API_KEY,
    });
    // Use the model name from env, or fallback to the provided modelId
    const actualModelName = process.env.LLM_MODEL || modelId.replace("openai/", "").replace("anthropic/", "");
    model = customOpenAI.chat(actualModelName);
    
    // Do not apply default provider options for custom LLMs to avoid unsupported features
    return model;
