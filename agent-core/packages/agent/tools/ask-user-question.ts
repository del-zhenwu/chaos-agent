...
const optionSchema = z.object({
  label: z
    .string()
    .describe("Choice text. Keep full technical identifiers (e.g. full pod names) when relevant."),
  description: z.string().describe("Explanation of trade-offs/implications"),
  value: z
    .string()
    .optional()
    .describe(
      "Optional canonical machine-readable value (e.g., cloud-staging/ai45-website-fe-deploy-xxx).",
    ),
});

const questionSchema = z.object({
  question: z.string().describe("The complete question to ask, ends with '?'"),
  header: z.string().max(24).describe("Short label for tab/chip display"),
