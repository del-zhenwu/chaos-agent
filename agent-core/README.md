...
## Chaos Agent Extension

This workspace includes a Chaos Agent extension with multi-engine support:

- `chaos-mesh` engine: compatible with Chaos Mesh Dashboard API and CRDs (`chaos-mesh.org/v1alpha1`) in Chaos Mesh **v2.6.x - v2.8.x**
- `chaosblade-k8s` engine: compatible with ChaosBlade Operator CRD (`chaosblade.io/v1alpha1`) in chaosblade-operator **v1.7.x - v1.8.x**

Runtime selection uses `CHAOS_ENGINE`:

- `CHAOS_ENGINE=chaos-mesh`
- `CHAOS_ENGINE=chaosblade-k8s`
